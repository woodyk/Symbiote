...
class DeceptionAnalyzer:
    ...
    def detect_contradictions(self, sentences):
        ...
        for i in range(len(sentences)):
            for j in range(i+1, len(sentences)):
                ...
                if sentences[i].strip().lower() == sentences[j].strip().lower():
                    continue  # Skip identical sentences
                if ('not' in sentences[i].lower() and 'not' not in sentences[j].lower()) or \
                   ('not' in sentences[j].lower() and 'not' not in sentences[i].lower()):
                    contradiction_count += 1
        contradiction_proportion = contradiction_count / total_pairs if total_pairs > 0 else 0
        return contradiction_proportion
    ...
    def analyze_entity_sentiment(self, text):
        ...
        avg_entity_sentiments = {entity: statistics.mean(sentiments) for entity, sentiments in entity_sentiments.items()}
        ...
        return sentiment_variance

    def count_temporal_references(self, words):
    ...
    def compute_deception_score(self, text):
        ...
        # New features
        contradiction_proportion = self.detect_contradictions(sentences)
        entity_sentiment_variance = self.analyze_entity_sentiment(text)
        temporal_reference_proportion = self.count_temporal_references(words)
        passive_voice_proportion = self.detect_passive_voice(sentences)
        ...
        motion_count = sum(1 for word in words if word in motion_verbs)
        ...
        weighted_scores = [weights[feature] * features[feature] for feature in features]
    ...
    def score(self, text):
        """
        Calculates the deception score of the provided text.

        Parameters:
        text (str): The text to analyze.

        Returns:
        float: Deception score between 0 and 1.
        """
