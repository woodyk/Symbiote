...
class LLMInteractor:
    def __init__(self, api_key=None, model="gpt-4o-mini", streaming=False, suppress=False):
        """
        Initialize the LLM Interactor. Supports OLLAMA
        ...
        for part in response:
            ...
            if part.choices[0].finish_reason == "function_call" and is_collecting_function_args:
                ...
                try:
                    ...
                    if self.api_key != "ollama":
                        input_data["functions"] = self.tools
...
def main():
    ...
    ola = LLMInteractor(api_key="ollama", model="llama3.2:latest")
    response = ola.handle_chat_request(
