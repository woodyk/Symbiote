...
# -*- coding: utf-8 -*-
#
# File: openai_function_calling.py
# Author: Wadih Khairallah
# Description: 
# Created: 2024-11-29 23:10:20
# Modified: 2025-03-14 13:31:56
...
def execute_python_code(code: str, confirm_execution: bool = False):
    """
    Execute the given Python code safely and return the output.

    Parameters:
        code (str): The Python code to execute.
        confirm_execution (bool): If False, it prompts for user confirmation before executing.

    Returns:
        dict: Execution result containing the output or error message.
    """
    # Display the code and ask for confirmation if required
    print("\nReceived Python code for execution:\n")
    print(code)
    print("\n")
    ...
    # Define a less restrictive but still safe execution environment
    safe_globals = {
        "__builtins__": {
            "print": print,
            "len": len,
            ...
            "any": any,
            "all": all,
            "open": open,  # Allow reading files (writing is risky)
            "__import__": builtins.__import__,  # Allow imports
        }
    }
...
def get_host_status():
    """
    Get the current status of the server, including uptime, CPU usage, memory usage, disk usage,
    load average, running processes, and network status.

    Returns:
        dict: A dictionary containing detailed server health and state information.
    """
    try:
        # Get system uptime
        ...
        # Get load average
        load_avg = os.getloadavg()  # Returns 1, 5, 15-minute load averages
        ...
        active_interfaces = {iface: details[0].address for iface, details in network_info.items() if details}
...
def run_bash_command(command: str):
    ...
    Returns:
        dict: Execution result, output, and potential errors.
    """
    try:
        # Run the command in a secure environment
...
def main():
    """
    Run an interactive chatbot loop that utilizes OpenAI's function calling.
    The user can interact with the chatbot in a conversational manner, and the chatbot
    will dynamically invoke functions as needed.
    """

    try:
        oai = OpenAIInteractor(model="gpt-4o")
    except Exception as e:
        print(f"Error loading Interactor: {e}")
        sys.exit()

    # Register available functions
    oai.add_function(external_callable=get_current_temperature)
    oai.add_function(external_callable=get_rain_probability)
    oai.add_function(external_callable=get_host_status)
    #oai.add_function(external_callable=execute_python_code)
    oai.add_function(external_callable=run_bash_command)

    print("Welcome to the OpenAI Function Calling Chatbot!")
    ...
    while True:
        ...
        # Print the chatbot's response (streamed output)
        for chunk in response:
            print(chunk, end="")  # Print each chunk as it arrives
        print()  # Ensure a new line after the response is fully received
